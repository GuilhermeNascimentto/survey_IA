...
def a_star(inicio, destino, grafo):
    ...
    while True:
        ...
        print(f"Visitando nó atual: {atual}")
 ...
 # remove o nó atual da lista de abertos e adiciona na lista de fechados
            ...
            abertos.remove((atual, prioridade(atual)))
            print(f"Nó atual removido da lista de abertos: ")
            fechados.add(atual)      
            print(f"Nó atual adicionado à lista de fechados: ")

        # para cada nó adjacente ao nó atual, calcula o custo atualizado
        for adjacente, custo in grafo[atual].items():  # correção aqui
            if adjacente in fechados:
                continue 

           
            if adjacente not in abertos or custo_g[atual] < custo_g[adjacente]:
                # atualiza o pai e o custo g do nó adjacente
                pai[adjacente] = atual
                custo_g[adjacente] = custo_g[atual]
                # adiciona o nó adjacente à lista de abertos com sua prioridade
                abertos.append((adjacente, prioridade(adjacente)))
                print(f"Nó adjacente adicionado à lista de abertos: ")
